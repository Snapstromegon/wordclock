...
function wc_update(){
	...
	var timestring = pack.timeString(date.getHours(),date.getMinutes(), wc_settings);
	...
	for(var i=0; i<html_wcs.length; i++){
		...
		for(var j=0; j<timewords.length; j++){
			...
			wc_setMode(letters,start+offset,start+offset+timewords[j].length, true);
...
function wc_setMode(domElems, start, end, mode){
	for(var i=start; i<end; i++){
		if(mode)
			domElems[i].classList.add(wc_brightLetterClassName);
		else
			domElems[i].classList.remove(wc_brightLetterClassName);
	}
}
...
function wc_getLanguagePack(langCode){
	for(var i=0; i<wc_languagePacks.length; i++){
		if(wc_languagePacks[i].langCode == langCode)
			return wc_languagePacks[i];
	}
	return false;
}
